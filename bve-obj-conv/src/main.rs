    ...
    for index_pair in polygon {
        ...
        let vertex_idx = if let Some(&existing_vert) = translation.get(&(position_idx, normal_idx, tex_coord_idx)) {
            ...
            *vert_count += 1;
            ...
            let position = obj.data.position[position_idx];
            let normal_opt = normal_idx.map(|idx| obj.data.normal[idx]);
            let texture_coords_opt = tex_coord_idx.map(|idx| obj.data.texture[idx]);
...
fn obj_to_csv(input_size: usize, obj: Obj) -> String {
    ...
    for object in &obj.data.objects {
        for group in &object.groups {
            ...
            for polygon in &group.polys {
                let indices = polygon_to_indices(&mut output, &mut vert_count, &obj, &mut translation, &polygon.0);
...
fn input_main() -> Option<()> {
    ...
    let parsed_object_result = ObjData::load_buf(&mut cursor)
